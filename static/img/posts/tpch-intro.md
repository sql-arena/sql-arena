---
title: "Introducing the TPC series - TPC-H Query 1: Column Storage and Local Aggregation"
date: "2025-07-14"
tags: 
  - "performance"
  - "query-planning"
...
After the wonderful feedback on the [previous blog about Iceberg](iceberg-is-wrong-2.md) - it is now time to switch 
gears.
...
- [Origins of the TPC and the first 10 years](https://www.tpc.org/information/about/history5.asp)

TPC benchmarks have a letter postfix,  the first one we will look at is TPC-H. This benchmark is a decision support
system - or what we call "data analytics", "data warehousing", "medallion architectures", "data lakes" or whatever
other word is in fancy today. Other analytical benchmarks preceded TPC-H. But, my limited time to blog will only allow us 
to travel back to 1999 - when TPC-H v1.0 was published.

## TPC-H Schema
The business domain of TPC-H is a retailer selling products. The schema uses an data model that is mostly in Third
Normal Form (3NF). While this method of data modelling is not popular today, many of the issues that TPC-H highlight
are every bit as relevant today - even in simpler data models.

A total of 8 tables make up the TPC-H schema. Two of these tables represent the core of the model, and take up the
majority of the space:

- `ORDERS` - contains information about customer orders, such as order date, ship date, and status.
- `LINEITEM` - related to `ORDERS` via a foreign key relation. Contains information about individual items in each 
  order, such as quantity, price, and discount.

The benchmark queries focus on operating on such a parent/child relationship between two large tables.

## TPC-H Queries
22 queries make up the TPC-H benchmark. The queries are relatively simple, which make them a good warmup exercise
until we get to TPC-DS - it's evil big brother.
...
I will be writing a series of blogs about TPC queries - starting today with TPC-H Query 1. I won't promise that we 
will analyse every single query. But, I will pick the most interesting ones and do a deep dive into the optimisations
that allow databases (or generally: code dealing with data) to run them efficiently.
...
The query is a simple aggregation on `l_returnflag` (cardinality 3) and `l_linestatus` (cardinality 2). This means
that the output result set is only 6 rows.
...
# Summary
Today, I analysed the first query of TPC-H. We saw that even an innocent looking query over a single table 
holds as surprising amount of optimisation potential. The algorithms we found useful were:
...
There is another optimisation I did not get to, namely using SIMD to handle the aggregate calculations. We will get to
that as we see more TPC queries in the future.

I hope this blog series is useful and educational - let me know what you think on LinkedIn. My hope is that I will have
time to write one of these blogs every week.
